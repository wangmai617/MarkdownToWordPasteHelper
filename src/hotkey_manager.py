"""
全局快捷键管理模块
注册全局快捷键Ctrl+Shift+V（也可自定义），键入时触发Markdown转换粘贴。
主要靠RegisterHotKey这个API实现，因为要全局监听，不能只用Qt的快捷键，否则只在应用激活时有效。
...
class HotkeyManager(QObject):
    ...
    def __init__(self, settings, tray, converter):
        super().__init__()
        ...
        self.modifiers = MOD_CONTROL | MOD_SHIFT  # 默认Ctrl+Shift
        self.vk_code = ord('V')     # 默认V键
    ...
    def modifiers_to_string(self):
        """返回修饰键的可读字符串，如'Ctrl+Shift'，给设置界面显示用"""
        parts = []
        if self.modifiers & MOD_CONTROL:
            parts.append('Ctrl')
        if self.modifiers & MOD_SHIFT:
