...
class TrayController(QWidget):
    ...
    def __init__(self, settings, converter):
        ...
        self.monitor = None          #剪贴板监控，后面通过set方法注入
        self.hotkey_manager = None   #热键管理器，同上
        # self.debug = False
    ...
    def set_monitor_and_hotkey(self, monitor, hotkey):
        """将外部创建的监控和热键对象传入，方便菜单操作"""
    ...
    def create_tray_menu(self):
        ...
        # 手动转换粘贴，与快捷键功能相同
        # 暂时保留。焦点不在Word/WPS时，托盘图标右键粘贴无法实现。
        ...
        #是否开启剪贴板监控（检测到Markdown时弹出提示）
        self.monitor_action = QAction("启用剪贴板监控", self)
        self.monitor_action.setCheckable(True)
        self.monitor_action.setChecked(self.settings.get_clipboard_monitor_enabled())
    ...
    def preview_markdown(self):
        ...
        #预览
        md_text = self.monitor.get_current_markdown()
        if not md_text:
    ...
    def toggle_monitor(self, checked):
        ...
        if self.monitor:
            if checked:
                ...
                self.monitor.stop()
            #保存到配置文件
            self.settings.set_clipboard_monitor_enabled(checked)
    ...
    def set_auto_start(self, enable):
        """将开机自启信息写入注册表（HKEY_CURRENT_USER下）（省去配置文件）"""
        import winreg
